...
cursor = con.execute("SELECT COUNT(*) FROM Locations where 0")
...
def addLink():
    ...
    if con.execute("SELECT COUNT(*) FROM Locations WHERE Name = ?",(loc1,)).fetchall()[0][0] != 0 and con.execute("SELECT COUNT(*) FROM Locations WHERE Name = ?",(loc2,)).fetchall()[0][0] != 0 and loc1 != loc2:
        ...
        return render_template("addLink.html", floors=floor(), userScrewedUp = True)
