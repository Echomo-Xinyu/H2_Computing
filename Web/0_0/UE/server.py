...
def deleteLink(q1, q2):
    ...
    if con.execute("SELECT COUNT(*) FROM Linking WHERE ['From'] = (?) AND ['To'] =  (?) ", (q1, q2)).fetchall()[0][0] != 0:
        con.execute(
            "DELETE FROM Linking WHERE ['From'] = (?) AND ['To'] = (?)", (q1, q2))
        con.execute(
            "DELETE FROM Linking WHERE ['From'] = (?) AND ['To'] = (?)", (q2, q1))
        con.commit()
        con.close()
        return True
    else:
...
def getLinks(query):
    ...
    con = openDB()
    cursor = con.execute(
        "SELECT ['To'] FROM Linking WHERE ['From'] = (?)", (query,))
...
def reader(query=None):
    ...
    if query == None:
        ...
        for i in query:
            cursor = con.execute(
                "Select * FROM Locations where Name = ?", (i,))
            rows += cursor.fetchall()
        print(rows)
    con.close()
    return rows
...
def addLink():
    ...
    if con.execute("SELECT COUNT(*) FROM Locations WHERE Name = ?", (loc1,)).fetchall()[0][0] != 0 and con.execute("SELECT COUNT(*) FROM Locations WHERE Name = ?", (loc2,)).fetchall()[0][0] != 0 and loc1 != loc2 and con.execute("SELECT COUNT(*) FROM Linking WHERE ['To'] = ? and ['From'] = ?", (loc1, loc2)).fetchall()[0][0] == 0:
        con.execute(
            "INSERT INTO Linking(['From'],['To']) VALUES (?, ?)", (loc1, loc2))
        print("hi")
        con.execute(
            "INSERT INTO Linking(['From'],['To']) VALUES (?, ?)", (loc2, loc1))
        con.commit()
    else:
...
def addItem():
    ...
    try:
        ...
        con.execute("INSERT INTO Locations(Name,Description,Image,Floor) VALUES (?,?,?,?)",
                    (request.form['name'], request.form['desc'], image_file_name, request.form['floor']))
        con.commit()
        con.close()
...
def search():
    ...
    con = openDB()
    res = con.execute("SELECT * FROM Locations WHERE Name LIKE ?",
                      ('%{}%'.format(query),)).fetchall()
    if auth:
...
def editItem(itemName):
    ...
    try:
        con.execute("UPDATE Locations SET Name = ?, Description = ?, Floor = ? WHERE Name = ?",
